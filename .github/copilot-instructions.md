...
Browser automation CLI that opens multiple internal dashboards (Tableau, SharePoint, JIRA, Azure, Power BI, Smartsheet, CloudZero, Atlassian) in a persistent Chromium session with SSO/token-based auth.
...
dashboard-agent/
  src/
    ...
    graph_report.py         # Generic graph report generator (orchestrator)
    ...
    graph_inputs.py         # Named graph input parsing and validation
    auth/
...
python run.py graph-report --graph "Name=/path/to/graph.png"  # Generate graph report
...
- **Graph report workflow**: capture screenshots or load local images → invoke `copilot -p --allow-all-tools` (streams until process exits, no hard timeout) → strip tool-activity preamble → generate HTML → display in browser
...
- **Context-based auth strategies (CloudZero, Atlassian) must run sequentially** — each opens its own tab and all leftover tabs are closed before the next service starts; running them concurrently causes logins to overwrite each other
- **`login_cloudzero` uses `wait_for_url()` with a 90 s timeout** — the SSO chain requires time to complete the redirect flow; timeout failures log the current URL instead of failing silently.
